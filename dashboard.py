...
pop_pib = carrega_parquet('./data/pop_pib_muni.parquet')
...
# COLUNAS
tabs = st.tabs(['UF do Brasil', 'Agro', 'América Latina'])
...
with tabs[1]:
    dados_susep = carrega_parquet('./data/susep_agro2.parquet')
    psr = carrega_parquet('./data/PSR_COMPLETO.parquet')
    ...
    dt_inicial_psr, dt_final_psr = col_config2.date_input('Data das Apólices', (date(2021, 1, 1), date(2021, 12, 31)), date(2006, 1, 7), date(2023, 12, 31), format="DD/MM/YYYY")
    ...
    with tabs_psr[0]:
        ...
        st.header(f'Número de Sinistros por Evento Climático de 2006 a 2023')
    ...
    with tabs_psr[1]:
        ...
        st.header(f'Sinistros de *{tipologia_selecionada_psr}* por estado de 2006 a 2023')
...
with tabs[2]:
    pop_pib_uf = carrega_parquet('./data/pop_pib_latam.parquet')
    malha_america = carrega_geojson('./data/malha_latam.json')
    malha_brasil = carrega_geojson('./data/malha_brasileira.json')
    coord_latam = carrega_parquet('./data/coord_latam3.parquet')
    ...
    idx_latam = dados_atlas.query("pais != 'Brasil'").index[0]
    heatmap_query_br = dados_atlas.iloc[idx_latam:].query("descricao_tipologia == @tipologia_selecionada_br & ano >= 2000")
